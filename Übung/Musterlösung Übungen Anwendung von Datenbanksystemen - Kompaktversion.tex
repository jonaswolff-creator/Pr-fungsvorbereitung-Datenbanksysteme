...
% ---------- Kompakt-Layout ----------
\renewcommand{\normalsize}{\fontsize{6.1}{6.6}\selectfont}
\normalsize
...
\titleformat{\subsubsection}{\bfseries\itshape\fontsize{6.1}{6.6}\selectfont}{}{0pt}{}
...
\subsection*{1.2--1.6 \quad Vollst\"andiges ER-Modell}
\begin{itemize}
\item Region$\to$Land$\to$Standort$\to$Abteilung$\to$Mitarbeiter: je 1:n-Kette
\item Mitarbeiter: rekursive 1:n ``ist Vorgesetzter von''
\item Projekt: \textbf{n:m} ``arbeitet mit'' Mitarbeiter (Beziehungsattr.\ Anteil, Projektleitung)
\item Job: 1:n ``ist angestellt als'' zu Mitarbeiter (Mindest-/Maximalgehalt abgeleitet)
\item Besch\"aftigungsverlauf: schwacher Entit\"atstyp, je 1:n zu Mitarbeiter/Abteilung/Job (keine Paarkombination eindeutig; PK sp\"ater: Mitarbeiter-ID+Start\_Datum)
\end{itemize}
...
\node[attr, below=of location, xshift=-1.4cm] (l-bl) {Bundesland};
...
\draw[kante] (region) -- (mid1); \draw[kante] (mid1) -- (country);
\kard{region}{mid1}{1} \kard{country}{mid1}{n}

\coordinate (mid2) at ($(country)!0.5!(location)$); \node[rel] at (mid2) {liegt in};
\draw[kante] (country) -- (mid2); \draw[kante] (mid2) -- (location);
\kard{country}{mid2}{1} \kard{location}{mid2}{n}

\coordinate (mid3) at ($(location)!0.5!(dept)$); \node[rel] at (mid3) {ist ange-\\siedelt in};
\draw[kante] (location) -- (mid3); \draw[kante] (mid3) -- (dept);
\kard{location}{mid3}{1} \kard{dept}{mid3}{n}
